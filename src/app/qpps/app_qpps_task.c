...
//            app_test_send_data(get_bit_num(app_qpps_env->char_status));
    ...
    __GucRFSendFlag = 0;
    ke_timer_clear(APP_SYS_SENDDATA_TIMER, TASK_APP);
    return (KE_MSG_CONSUMED);
}
...
//            ke_timer_set(APP_SYS_ALARM_START_TIMER, TASK_APP, 2);                
            ke_timer_set(APP_SYS_OPEN_TIMER, TASK_APP, 2);   
...
//            MOTOR_OPEN();
//            ke_timer_set(APP_SYS_ALARM_START_TIMER, TASK_APP, 2);                
            ke_timer_set(APP_SYS_CLOSE_TIMER, TASK_APP, 2);                
